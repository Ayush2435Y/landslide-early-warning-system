...
export const TopNavbar: React.FC<TopNavbarProps> = ({
  ...
  const isUserAdmin = Boolean(!isGuest && (currentUser?.role === 'admin' || currentUser?.role === 'super_admin'));
  ...
  return (
    <header className="w-full h-16 shrink-0 z-30 flex justify-between items-center px-4 md:px-6 bg-[#0a1128] border-b border-slate-800 text-white select-none shadow-md">
      ...
      <div className="flex items-center gap-2.5 md:gap-4">
        ...
        {/* Disaster Protocol Emergency Button */}
        <button
          onClick={onOpenEmergency}
          className="px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all bg-red-600/90 hover:bg-red-600 text-white border border-red-500 shadow-xs cursor-pointer"
          title="Open Disaster Protocol & SOP Checklists"
        >
          <ShieldAlert className="w-4 h-4 text-white animate-pulse" />
          <span className="hidden sm:inline">Disaster Protocol</span>
        </button>
        ...
        {/* Direct Quick Sign In button for Guests */}
        {isGuest && (
          <button
            onClick={() => onOpenAuthModal?.('user')}
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-xs cursor-pointer"
            title="Sign in with your Mobile OTP or Admin Credentials"
          >
        ...
        {/* User Profile / Guest Account Badge */}
        <div className="relative">
          <button
            onClick={() => setShowUserDropdown(!showUserDropdown)}
            className="flex items-center gap-2.5 pl-2 pr-3 py-1.5 rounded-lg hover:bg-slate-800/80 transition-colors cursor-pointer"
          >
            ...
            <div className="hidden lg:flex flex-col text-left">
              ...
              <span className="text-[10px] text-slate-400">
                {isUserAdmin 
                  ? 'Administrator (HQ)' 
                  : isGuest 
                    ? 'Guest Mode • Click to Sign In' 
                    : 'Civilian User (Verified)'}
              </span>
            </div>
            ...
            <div className="absolute right-0 mt-2 w-72 bg-[#0d1630] border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50 text-slate-200 text-xs py-1 animate-in fade-in slide-in-from-top-2">
              <div className="px-3.5 py-2.5 border-b border-slate-700/80 bg-[#080f24]">
                <div className="flex items-center justify-between">
                  <p className="font-bold text-white">{isGuest ? 'Guest Account' : currentUser?.name}</p>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded font-semibold ${
                    ...
                    {isUserAdmin ? 'HQ ADMIN' : isGuest ? 'GUEST MODE' : 'VERIFIED CITIZEN'}
                  </span>
                </div>
                <p className="text-[11px] text-slate-400 truncate mt-0.5">
                  {isGuest ? 'Public land hazard view mode' : currentUser?.email}
                </p>
                {!isGuest && currentUser?.phone && (
              ...
              {isGuest ? (
                <>
                  <button
                    ...
                    className="w-full px-3.5 py-2.5 text-left bg-emerald-950/40 hover:bg-emerald-900/50 text-emerald-300 font-bold transition-colors flex items-center gap-2.5"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3.5 py-2 text-left hover:bg-slate-800 text-blue-400 font-medium transition-colors flex items-center gap-2.5 border-t border-slate-800"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3.5 py-2 text-left hover:bg-slate-800 transition-colors flex items-center gap-2.5 border-t border-slate-800"
                  >
                ...
                <>
                  ...
                  {onLogout && (
                    <button
                      ...
                      className="w-full px-3.5 py-2 text-left hover:bg-rose-950/30 text-rose-300 hover:text-rose-200 transition-colors flex items-center gap-2.5 border-t border-slate-800"
                    >
                ...
                <>
                  ...
                  <button
                    ...
                    className="w-full px-3.5 py-2 text-left hover:bg-slate-800 text-blue-400 transition-colors flex items-center gap-2.5 border-t border-slate-800"
                  >
                    <KeyRound className="w-3.5 h-3.5 text-blue-400" />

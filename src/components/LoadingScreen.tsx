...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Bulletproof failsafe watchdog: If app takes longer than 3.5s for any reason (e.g. Render CPU throttling), force complete
    const failsafeWatchdog = setTimeout(() => {
      clearInterval(timer);
      if (!isFadingOut) {
        setProgress(100);
        setIsFadingOut(true);
    ...
    // Keyboard listener for instant enter/escape/space dismissal
    const handleKeyDown = (e: KeyboardEvent) => {
      if (['Enter', 'Escape', ' ', 'ArrowRight'].includes(e.key)) {
        clearInterval(timer);
        clearTimeout(failsafeWatchdog);
        setIsFadingOut(true);
        setTimeout(() => {
          onComplete();
        }, 200);
  ...
  }, [minDisplayTimeMs, onComplete, isFadingOut]);

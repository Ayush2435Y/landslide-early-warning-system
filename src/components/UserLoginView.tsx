...
export const UserLoginView: React.FC<UserLoginViewProps> = ({
  ...
  const isCitizenLoggedIn = Boolean(
    currentUser &&
    currentUser.isAuthenticated &&
    !isGuestUser(currentUser) &&
    (currentUser.role === 'citizen' || currentUser.role === 'field_officer' || currentUser.role === 'user')
  );
                    ...
                    <button
                      ...
                      onClick={() => {
                        const verifiedUser: AuthenticatedUser = {
                          ...
                          token: 'otp_auto_verified_' + Math.random().toString(36).substring(7),
                          loginTime: new Date().toISOString(),
                          isAuthenticated: true,
                        };
                        onLoginSuccess(verifiedUser);
                        onShowToast('Signed in as Ayush Paul (Civilian)');
                      }}
                      className="text-xs text-blue-700 hover:underline flex items-center gap-1 cursor-pointer font-medium"
                    >

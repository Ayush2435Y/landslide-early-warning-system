...
export const CitizenDashboardView: React.FC<CitizenDashboardViewProps> = ({
  ...
  const isAuthenticated = Boolean(currentUser && currentUser.isAuthenticated && currentUser.id !== 'guest-user' && currentUser.id !== 'guest-public');
  ...
  return (
    <div className="flex-1 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-6 animate-in fade-in duration-200">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold flex items-center gap-1 border ${
                ...
                {isAuthenticated ? (
                  <>
                    ...
                    <span>Civilian Account • Designated Citizen Dashboard</span>
                  </>
                ) : (
                  <>
                    <Eye className="w-3.5 h-3.5 text-slate-600" />
                    <span>Guest Mode • Public Landslide Viewer</span>
                  </>
                )}
              </span>
            ...
            <h1 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight">
              Welcome, {isAuthenticated ? (currentUser?.name || 'Citizen') : 'Guest Observer'}
            </h1>
      ...
      {/* 2. Admin Feature Restriction Banner / Guest Sign In Banner */}
      <div className="bg-amber-50/90 border border-amber-200/90 rounded-2xl p-4 text-xs flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-2xs">
          ...
          <div>
            <span className="font-bold text-amber-950 block">
              {isAuthenticated 
                ...
                : 'Guest Public Access Mode Active'}
            </span>
            <span className="text-amber-800 text-[11px]">
              {isAuthenticated
                ...
                : 'You are viewing public hazard bulletins. Sign in via Phone/Email OTP or Admin credentials to unlock full citizen or officer reporting capabilities.'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!isAuthenticated && (
            <button
              onClick={() => onOpenAuthModal?.('user')}
              className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-slate-900 hover:bg-black text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          >

...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  // Admin auth gate state for civilian role
  const [adminEmail, setAdminEmail] = useState('admin@gsi.gov.in');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-[#fcf8fa] w-full max-w-2xl rounded-2xl border border-[#c6c6cd] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {!isAdmin ? (
          /* Civilian Access Restricted Clearance Gate */
          <div className="p-6 md:p-8 flex flex-col items-center justify-center text-center space-y-5 flex-1 bg-white overflow-y-auto">
            <div className="w-16 h-16 rounded-3xl bg-amber-100 border border-amber-300 flex items-center justify-center text-amber-700 shadow-inner">
            ...
            <div className="max-w-md space-y-1.5">
              ...
              <h3 className="text-base font-bold text-slate-900">
                Configuration Tools Locked
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                System and GIS configuration tools (datum calibration, telemetry polling frequency, edge offline cache rules, and sensor alert thresholds) are strictly restricted from the civilian user role.
              </p>
            </div>

            {adminAuthError && (
              <div className="w-full max-w-sm p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex items-center gap-2 text-left">
                <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0" />
                <span>{adminAuthError}</span>
              </div>
            )}

            <form onSubmit={handleAdminAuthenticate} className="w-full max-w-sm space-y-3.5 text-left">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs text-slate-900 font-mono"
                    required
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-xs disabled:opacity-50"
                >
                  {adminAuthLoading ? 'Verifying...' : 'Unlock Config'}
                </button>
              </div>
            </form>
          </div>

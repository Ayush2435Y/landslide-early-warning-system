...
export const DEFAULT_CIVILIAN_USER: AuthenticatedUser = {
  id: 'citizen-907',
...
// Guest user profile (Public Observer Mode when signed out)
export const GUEST_USER: AuthenticatedUser = {
  id: 'guest-user',
  name: 'Guest Observer',
  email: 'guest@bhumirakshak.gov.in',
  phone: '',
  role: 'citizen',
  department: 'Public Guest Observer',
  jurisdiction: 'Public Landslide Warning Viewer',
  isConfidentialCleared: false,
  token: '',
  loginTime: new Date().toISOString(),
...
export function App() {
  ...
  const handleLogout = useCallback(() => {
    ...
    setCurrentTab('dashboard');
    showToast('Signed out successfully. Switched to Guest Account. You can sign in again anytime.');
  }, [showToast]);
  ...
  return (
    <div className="h-screen w-full max-w-full bg-[#edf2f7] text-slate-900 flex flex-col font-sans overflow-hidden selection:bg-emerald-200">
          ...
          {currentTab === 'sensors' && (
            isAdmin ? (
              ...
              <AdminAccessGuard
                ...
                onReturnToCitizenDashboard={() => setCurrentTab('dashboard')}
                onShowToast={showToast}
              />
            )
          )}
      ...
      <EmergencyProtocolModal
        ...
        sensors={sensors}
        currentRole={isAdmin ? 'admin' : 'citizen'}

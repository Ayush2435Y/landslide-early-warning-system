...
export const GUEST_USER: AuthenticatedUser = {
  id: 'guest-user',
  name: 'Guest User',
  email: 'guest@ner.gov.in',
  role: 'citizen',
  department: 'Public Civilian Access (Guest)',
  jurisdiction: 'NER Public Access',
...
export const isGuestUser = (user?: AuthenticatedUser | null): boolean => {
  if (!user) return true;
  return !user.isAuthenticated || user.id === 'guest-user';
};
